...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/sbabtiwaHW5"
## Provided:
...
class UpdatePriority(FlaskForm): 
    newPriority = StringField("What is the new priority of the TODO list item?", validators=[Required()])
    submit = SubmitField("Update")
...
def delete(lst):
    ...
    db.session.delete(d)
    db.session.commit()
    flash("Successfully deleted {}".format(d.title))
    return redirect(url_for('all_lists'))
